...
func TestLeblad_GetPhoneCodesForWilaya(t *testing.T) {
	type args struct {
		wilayaName string
	}
	tests := []struct {
		...
		{
			name:    "GetPhoneCodesForWilaya for existing wilaya name",
			l:       New(),
			args:    args{wilayaName: "Adrar"},
			wantErr: false,
		},
		{
			name:    "GetPhoneCodesForWilaya for non-existing wilaya name",
			l:       New(),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			l := tt.l
			_, err := l.GetPhoneCodesForWilaya(tt.args.wilayaName)
			if (err != nil) != tt.wantErr {
				t.Errorf("Leblad.GetPhoneCodesForWilaya() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
		})
	}
...
func TestLeblad_GetBaladyiatsForWilaya(t *testing.T) {
	type args struct {
		wilayaName string
		fields     []string
	}
	tests := []struct {
		...
		{
			name:    "GetBaladyiatsForWilaya for existing wilaya name",
			l:       New(),
			args:    args{wilayaName: "Adrar"},
			wantErr: false,
		},
		{
			name:    "GetBaladyiatsForWilaya for non-existing wilaya name",
			l:       New(),
			args:    args{wilayaName: "Non existing wilaya"},
			wantErr: true,
		},
		{
			name:    "GetBaladyiatsForWilaya for existing wilaya name with fields",
			l:       New(),
			args:    args{wilayaName: "Adrar", fields: []string{"name"}},
			wantErr: false,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			l := tt.l
			_, err := l.GetBaladyiatsForWilaya(tt.args.wilayaName, tt.args.fields...)
			if (err != nil) != tt.wantErr {
				t.Errorf("Leblad.GetBaladyiatsForWilaya() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
		})
	}
...
func TestLeblad_GetDairaByBaladyiaName(t *testing.T) {
	type args struct {
		baladyiaName string
		fields       []string
	}
	tests := []struct {
		...
		{
			name:    "GetDairaByBaladyiaName for existing baladyia name",
			l:       New(),
			args:    args{baladyiaName: "ADRAR"},
			wantErr: false,
		},
		{
			name:    "GetDairaByBaladyiaName for non-existing baladyia name",
			l:       New(),
			args:    args{baladyiaName: "Non existing baladyia"},
			wantErr: true,
		},
		{
			name:    "GetDairaByBaladyiaName for existing baladyia name with fields",
			l:       New(),
			args:    args{baladyiaName: "ADRAR", fields: []string{"name"}},
			wantErr: false,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			l := tt.l
			_, err := l.GetDairaByBaladyiaName(tt.args.baladyiaName, tt.args.fields...)
			if (err != nil) != tt.wantErr {
				t.Errorf("Leblad.GetDairaByBaladyiaName() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
		})
	}
